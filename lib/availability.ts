/**
 * Availability computation — figures out when Brad is free to take on
 * new work, based on his real schedule.
 *
 * Used by the lakeside public availability API which the
 * painterswanaka.co.nz site reads to render the "next available"
 * banner. Lives on lakeside (Brad-specific marketing automation), not
 * main — though the algorithm is general enough to port if it proves
 * useful for other painters later.
 *
 * ── Design notes ──────────────────────────────────────────────────────
 *
 ...
 *   honest: it represents work Brad has SAID YES to, not work he might
 *   eventually win.
 *
 ...
 *   - Holidays / personal time. Brad would have to put those in the
 *     schedule manually as accepted-status jobs or 'reminder' items if
 *     he wanted them excluded.
...
export interface AvailabilityReport {
  ...
  /**
   * Subsequent occupied stretches Brad is committed to — useful for
   * the banner to say "...then booked 6-12 July, then free again."
   * Includes the job's name for tooltip display.

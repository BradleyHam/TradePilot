...
//
// Public read-only endpoint that returns the configured business's
// current and upcoming schedule availability, computed from real
// schedule_items + jobs in Supabase. Designed to be fetched by the
// business's customer-facing website (e.g. painterswanaka.co.nz) to
// render a "next available" banner.
//
// Why public, no auth:
//   The data leaked here is just "I'm/I'm-not free, here are some
//   dates" — which a tradie would tell any prospective client over
//   the phone anyway. No financials, no client names beyond the
//   booking's job name (which is typically the address, e.g.
//   "84C Aubrey Road"). If that ever becomes a privacy concern,
//   jobName can be stripped.
//
// Why service role:
...
//   endpoint only ever talks about the one whose ID is set on this
//   deployment.
//
// Caching:

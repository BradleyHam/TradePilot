// POST /api/marketing/instagram-caption
//
// Auth-gated. Body: { jobId, description?, overview?, services? }. Instagram
// twin of facebook-caption: reshapes the reviewed website copy + job facts
// into an IG-flavoured caption — shorter, no link (not clickable on IG),
// heavier hashtags. Nothing persisted here; the preview shows it for review.
//
// Returns:
...
import { generateInstagramPost } from '@/lib/marketing-ai';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const post = await generateInstagramPost({
      ...
      // No projectUrl on purpose — links aren't clickable on Instagram.
    });

    return NextResponse.json({ ok: true, caption: post.caption });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[marketing/instagram-caption]', msg);

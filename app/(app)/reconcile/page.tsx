...
import { parseBnzCsv, type ParsedBankRow } from '@/lib/bank-csv';
import { classifyBankRow, findMatchingEntry, type Suggestion } from '@/lib/bank-classifier';
import { rankJobs, type RankedJob } from '@/lib/job-match';
import type { BankTransaction, Entry, ExpenseCategory } from '@/lib/types';
import { Upload, CheckCircle2, ArrowRight, X, AlertCircle, Receipt, DollarSign } from 'lucide-react';
...
export default function ReconcilePage() {
  const {
    ...
    importBankTransactions, updateBankTransaction,
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      // Bring parsed rows into the bank-transaction shape; default to
      // unreconciled. The store handles the business_id and timestamp.
      ...
      <div className="px-4 md:px-6 pb-6 space-y-3">
        ...
        <div className="bg-card border border-border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-semibold text-foreground">Import BNZ CSV</p>
            {importing && (
          ...
          <label className="flex items-center gap-2 px-3 h-10 rounded-lg border border-dashed border-input bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer">
...
function NewEntryForm({
  ...
  const [isOverhead, setIsOverhead] = useState<boolean>(false);

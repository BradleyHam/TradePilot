  ...
  return (
    <div>
      ...
      <Sheet open={mode?.kind === 'add' || mode?.kind === 'edit'} onOpenChange={(open) => !open && setMode(null)}>
        <SheetContent side="bottom" className="h-[85vh] overflow-y-auto rounded-t-2xl pb-10">
      ...
      <Sheet open={mode?.kind === 'split'} onOpenChange={(open) => !open && setMode(null)}>
        <SheetContent side="bottom" className="h-auto max-h-[85vh] overflow-y-auto rounded-t-2xl pb-10">
          <SheetHeader className="pb-4">
